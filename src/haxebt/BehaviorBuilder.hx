...
class BehaviorBuilder {
    ...
    public static macro function build():Array<Field> {
        ...
        if(toInit.length > 0) {
            ...
            fields.push({
                pos: Context.currentPos(), name: 'new', access: [APublic],
        ...
        fields.push({
            pos: Context.currentPos(), name: 'getInfo', access: [APublic, AOverride], meta: [{ name: ':keep', pos: Context.currentPos() }],
    ...
    public static var behaviorBaseArgs:Array<FunctionArg> = [
        ...
        { name: 'id', type: macro :haxebt.BehaviorForest.BehaviorNodeID },
        { name: 'sibling', type: macro :haxebt.BehaviorForest.BehaviorNodeID },
        { name: 'child', type: macro :haxebt.BehaviorForest.BehaviorNodeID }
    ];
    ...
    private static function buildRun(execute:Function, entityType:Type, pos:Position):Field {
        ...
        var dataInit = macro { };
        if(execute.args.length > 2) Context.error('Unexpected amount of arguments, use data and/or entity, no more.', pos);
        var dataArg = null;
        ...
        #if ai_debug
        var debugExpr = macro tools.AISnapshot.enter(this.id, $i{entityArgName});
        #else
        var debugExpr = macro { };
        #end
        return {
            ...
            kind: FFun({
                args: [
                    { name: 'storage', type: macro :Map<haxebt.BehaviorForest.BehaviorNodeID, Dynamic> },
    ...
    private static function getDataInit(dataName:String, dataType:ComplexType, pos:Position):Expr {
        ...
        var wantsEntity = initField != null
    ...
    private static function addDebugToReturn(e:Expr):Expr {
        return switch(e.expr) {
            case EReturn(e):
                macro @:mergeBlock {
                    final result:Behavior.BehaviorResult = $e;
                    tools.AISnapshot.exit(this.id, result, $i{entityArgName});
                    return result;
                }
    ...
    private static function addClearToReturn(e:Expr):Expr {
        return switch(e.expr) {
            case EReturn(e):
                macro @:mergeBlock {
                    final result:Behavior.BehaviorResult = $e;
                    if(result != Behavior.BehaviorResult.Running) {
